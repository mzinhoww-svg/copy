...
# ReinersMedia — Design System (foundations)

A foundations-only design system for **Reiners Media**: colour, typography,
spacing, geometry, and a linear icon language. There are no React components —
you compose layouts with the tokens. Link one stylesheet and build.

## Where things are

- `styles.css` — the single stylesheet entry. Link this one file; it `@import`s the fonts, all tokens, and a sober base layer. Rendered designs receive only this file's `@import` closure.
- `tokens/colors.css` · `tokens/typography.css` · `tokens/spacing.css` — CSS custom properties, values verbatim from the Brand Guideline.
- `fonts/` — self-hosted woff2 (`@font-face` in `fonts.css`) for Cormorant Garamond, DM Sans, DM Mono, so designs render on-brand with no network dependency.
- `components/<group>/<Name>/<Name>.html` — browsable reference cards (Colors, Type, Spacing, Geometry, Icons, Brand). `<Name>.prompt.md` holds the rule for each.
- `_ds_bundle.js` — an empty-bodied bundle (this DS ships no components); present only to satisfy the loader.
...
That is the whole setup — no script, no React provider.

## Tokens

CSS custom properties from the Reiners Media Brand Guideline (Edição 02). Names
are the source of truth — see `tokens/` for the full list and each card's
`.prompt.md` for usage rules.

- **color**: `--navy`, `--ouro`, `--creme`, `--tinta`, + 6 support tones and semantic aliases
- **typography**: `--serif`, `--sans`, `--mono`, size/weight/leading/tracking scales
- **spacing**: `--space-1`…`--space-10` (8px base), grid + layout tokens
